...
export function buildSystemPrompt(tools: ToolDefinition[], maxIterations: number): string {
  ...
  return [
    ...
    "Si las herramientas de Google Workspace estan registradas, nunca respondas que no tienes acceso o que hace falta instalar gog sin haber intentado primero la herramienta adecuada. Solo explica setup si la herramienta falla realmente por configuracion o permisos.",
    "Para Gmail, usa gmail_search_messages y aprovecha inbox_section cuando aplique: principal=primary, promociones=promotions, social=social, notificaciones o actualizaciones=updates y foros=forums.",
    "Si el usuario pide el ultimo correo, normalmente debes usar gmail_search_messages con max_results=1.",
    "Si hay una nota persistente que diga que por defecto quieres la bandeja principal, aplica inbox_section=primary siempre que el usuario pida correos sin especificar categoria.",
    "Si el usuario responde con algo corto como 'si', 'vale', 'ok', 'dime mas', 'cual?' o 'que evento', interpreta que se refiere a la ultima entidad que acabas de mencionar y continua desde ahi. Si hace falta, vuelve a consultar la misma herramienta para ampliar detalles.",
